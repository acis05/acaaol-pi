...
function showResultPanel(summary = {}, results = []) {
  ...
  if (failedItems.length > 0) {
    const lines = [];
    lines.push("CATATAN GAGAL IMPORT PURCHASE INVOICE");
    lines.push("==================================");
    failedItems.forEach((x, idx) => {
      lines.push("");
      lines.push(`${idx + 1}. Invoice: ${x.number || "-"} | Tanggal: ${x.transDate || "-"} | Vendor: ${x.vendorNo || "-"}`);
...
if ($("btnBuild")) {
  $("btnBuild").onclick = async () => {
    try {
      ...
      const res = await postForm("/api/build-purchase-invoice", fd);
...
if ($("btnImport")) {
  $("btnImport").onclick = async () => {
    try {
      ...
      setSummary("Sedang mengimport Purchase Invoice ke Accurate...");
      const res = await postJson("/api/import-purchase-invoice", { payload: builtPayload }, true);
